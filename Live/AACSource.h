#ifndef __AACSOURCE__H_
#define __AACSOURCE__H_
...
class AACSource : public Source
{
public:
	AACSource(UsageEnvironment* env, const std::string& file);
	virtual ~AACSource();
	static AACSource* createNew(UsageEnvironment* env, const std::string& file);
...
private:
	...
	bool parseAdtsHeader(uint8_t* buf, struct AdtsHeader* res);

	FILE* _file;
	struct AdtsHeader _adtsHeader;
};
